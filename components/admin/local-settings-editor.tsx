    ...
    <div>
      <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        ...
        <button
          ...
          className="rounded-xl bg-white/10 px-4 py-3 text-sm font-bold text-white sm:self-auto"
        >
          Restaurar
        </button>
      </div>

      <div className="mt-6 grid min-w-0 gap-4 rounded-2xl border border-white/10 bg-white/6 p-4 sm:rounded-3xl sm:p-5">
        {fields.map((field) => (
          <label key={field.key} className="grid min-w-0 gap-2 text-sm font-bold text-white/70">
            {field.label}
            {field.type === "textarea" ? (
              <textarea
