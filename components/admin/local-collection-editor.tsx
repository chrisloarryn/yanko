  ...
  return (
    <div>
      ...
      <div className="mt-6 grid gap-5 xl:grid-cols-[0.9fr_1.1fr]">
        <div className="max-h-80 overflow-auto rounded-2xl border border-white/10 sm:rounded-3xl xl:max-h-none">
          {items.map((item, index) => {
            ...
            return (
              <button
                ...
                className={`grid w-full min-w-0 gap-2 border-b border-white/10 p-4 text-left text-sm transition last:border-b-0 ${
              ...
              >
                ...
                {secondary ? <span className="break-words text-xs text-white/45">{secondary}</span> : null}
              </button>
            );
          })}
        </div>

        <div className="min-w-0 rounded-2xl border border-white/10 bg-white/6 p-4 sm:rounded-3xl sm:p-5">
          {selectedItem ? (
            <div className="grid gap-4">
              {fields.map((field) => {
                ...
                if (field.type === "select") {
                  return (
                    <label key={String(field.key)} className="grid min-w-0 gap-2 text-sm font-bold text-white/70">
                      {field.label}
                      <select
                        value={String(value)}
                        onChange={(event) => updateItem(field.key, event.target.value)}
                        className="min-w-0 rounded-2xl border border-white/10 bg-slate-950 px-4 py-3 text-white outline-none focus:border-emerald-500"
                      >
                  ...
                  return (
                    <label key={String(field.key)} className="grid min-w-0 gap-2 text-sm font-bold text-white/70">
                      ...
                      <textarea
                        value={String(value)}
                        onChange={(event) => updateItem(field.key, event.target.value)}
                        className="min-h-28 min-w-0 rounded-2xl border border-white/10 bg-slate-950 px-4 py-3 text-white outline-none focus:border-emerald-500"
                      />
                    </label>
                  );
                }
      ...
      <details className="mt-6 rounded-2xl border border-white/10 bg-white/6 p-4 sm:rounded-3xl sm:p-5">
